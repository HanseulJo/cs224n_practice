...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            raise NotImplementedError(f"invalid transition: {transition}")
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    #print("<minibatch parsing>")
    #print(sentences[31])
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    unfinished_parses = partial_parses[:]
    while len(unfinished_parses) > 0:
        transitions = model.predict(unfinished_parses[:batch_size])
        for i in range(len(unfinished_parses[:batch_size]))[::-1]:
            unfinished_parses[i].parse_step(transitions[i])
            if (len(unfinished_parses[i].stack), len(unfinished_parses[i].buffer)) == (1, 0):
...
def test_parse():
    ...
    dependencies = PartialParse(sentence).parse(['S', 'S', 'LA', 'S', 'S', 'LA', 'RA', 'S', 'RA', 'RA'])
    dependencies = tuple(sorted(dependencies))
    #expected = (('ROOT', 'parse'), ('parse', 'sentence'), ('sentence', 'this'))
    expected = (('ROOT', 'parsed'), ('parsed', 'I'), ('parsed', 'correctly'), ('parsed', 'sentence'), ('sentence', 'this'))
    assert dependencies == expected,  \
        "parse test resulted in dependencies {:}, expected {:}".format(dependencies, expected)
    #assert tuple(sentence) == ("parse", "this", "sentence"), \
    assert tuple(sentence) == ('I', 'parsed', 'this', 'sentence', 'correctly'), \
