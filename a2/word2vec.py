...
def sigmoid(x):
    ...
    s = np.piecewise(x.astype('float') if type(x)==np.ndarray else float(x), [x>=0], [lambda y: 1/(1+np.exp(-y)), lambda z: 1-(1/(1+np.exp(z)))]) # stable
    ### END YOUR CODE
...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    vocabSize, vectorDim = outsideVectors.shape
    yPred = softmax(outsideVectors @ centerWordVec)
    yTrue = np.eye(1, vocabSize, outsideWordIdx)[0]
    loss = -np.log(yPred[outsideWordIdx])
    gradCenterVec = (yPred - yTrue) @ outsideVectors
    gradOutsideVecs = np.diag(yPred - yTrue) @ np.tile(centerWordVec, (vocabSize,1))
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    sample = outsideVectors[indices]
    sigmoidProd = sigmoid(sample @ centerWordVec)
    loss = -np.log(sigmoidProd[0]) - np.log(1-sigmoidProd[1:]).sum()
    gradCenterVec = (sigmoidProd - np.eye(1,K+1,0)[0]) @ sample
    gradOutsideVecs = np.zeros_like(outsideVectors)
    for i in range(K+1):
        gradOutsideVecs[indices[i]] += (sigmoidProd[i] - (0 if i else 1)) * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outsideWordsInds = [word2Ind[word] for word in outsideWords]
    currentCenterVector = centerWordVectors[currentCenterInd]
    Ls, GCVs, GOVs = zip(*[word2vecLossAndGradient(currentCenterVector, ind, outsideVectors, dataset) for ind in outsideWordsInds])
    loss = np.sum(Ls)
    gradCenterVecs[currentCenterInd] = np.sum(GCVs, axis=0)
    gradOutsideVectors = np.sum(GOVs, axis=0)
    ### END YOUR CODE
